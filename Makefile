...
# defaulted below. However, you might want to set this to 'podman'
...
test-unit: ## Run unit tests only.
	go test ./internal/... ./cmd/... -v -race -coverprofile=unit-cover.out

.PHONY: test-integration
test-integration: manifests generate envtest ## Run integration tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test ./test/integration/... -v -coverprofile=integration-cover.out
...
.PHONY: coverage-func
coverage-func: test ## Show function-level test coverage.
...
# To properly provided solutions that are compatible with more than one architecture, you should use this option.
